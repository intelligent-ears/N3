...
class RiskCalculator {
     ...
     */
    calculateTemplateRisk(template, findings) {
        ...
        if (template.risk_calculation.modifiers) {
            ...
            if (findings.length === 0) {
                ...
                score += modifierValues.reduce((sum, val) => sum + val, 0);
            }
        }
    ...
    calculateOverallRisk(results) {
        if (results.length === 0)
            return 100; // No scans = perfect score
    ...
    generateSummary(results) {
        ...
        results
            ...
            .forEach((result) => {
            const severity = result.template.severity;
            summary[severity]++;
        });
        return summary;
    }
